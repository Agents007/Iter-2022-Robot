...
const int capteurGauche = 81;
const int capteurDroit = 80;

const int buttoncalibrate = 33;
const int buttonstart = 32;

const int nombreInter = 0;
const int tableauInter[nombreInter]{};
int compteur = 0;
int compteurInf = 0;
  ...
  if (onoff == true) {
    if (digitalRead(capteurGauche) == false){
      if (tableauInter[compteur] == 0){
         while (digitalRead(capteurGauche) == false && sensorsValues[3]){
          sens = 5500;
          PID_control(sens);
        }
      }
    ...
    else if (digitalRead(capteurDroit) == false){
      if (tableauInter[compteur] == 1){
        while (digitalRead(capteurDroit) == false && sensorsValues[4]){
          sens = 1500;
          PID_control(sens);
        }
      }
